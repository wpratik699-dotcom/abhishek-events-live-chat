...
const SYNC_ADMIN_PASSWORD = process.env.SYNC_ADMIN_PASSWORD === "true";
...
const adminUser = db
  .prepare("SELECT * FROM users WHERE username='admin' AND role='admin'")
  .get();
...
function chooseAgent() {
  ...
  const candidates = agents
    ...
    .filter(agent => agent.current < agent.capacity)
    .sort(
      (a, b) =>
        (a.current - b.current) ||
        (a.id - b.id)
...
function conversationAllowed(user, conversationId) {
  ...
  if (
    user.role === "customer" &&
    conversation.customer_token !== user.token
  ) {
    return null;
  }
...
app.post(
  ...
  (req, res) => {
    ...
    const info = db.prepare(`
      ...
      (customer_name, customer_token, status)
      VALUES (?, ?, 'waiting')
    `).run(
      customerName,
      token
...
server.listen(PORT, () => {
  console.log(
    `Abhishek Events server running at http://localhost:${PORT}`
  );
});
